...
# Create identical features (604 dimensions)
import re
...
# Rebuild target encoder on train data for safety
y = np.log1p(df_reg["salary_mid"].values)
idx = np.arange(len(df_reg))
salary_q = pd.qcut(df_reg["salary_mid"], q=5, labels=False)
idx_t, idx_temp, y_t, y_temp = train_test_split(idx, y, test_size=0.3, random_state=42, stratify=salary_q)
idx_v, idx_test, y_v, y_test = train_test_split(idx_temp, y_temp, test_size=0.5, random_state=42, stratify=salary_q.iloc[idx_temp])

idx_train_val = np.concatenate([idx_t, idx_v])
y_train_val = np.concatenate([y_t, y_v])

te_cols = [c for c in ['location', 'industry'] if c in df_reg.columns]
te = ce.TargetEncoder(cols=te_cols, smoothing=20)
te.fit(df_reg.loc[idx_t, te_cols], y_t)
joblib.dump(te, os.path.join(MODELS_DIR, 'target_encoder.joblib'))

df_reg_enc = df_reg.copy()
df_reg_enc[[c + '_te' for c in te_cols]] = te.transform(df_reg[te_cols])
...
X_train_val = X_all[idx_train_val]

print("Training XGBoost Regressor...")
xgb_model = xgb.XGBRegressor(n_estimators=600, learning_rate=0.04, max_depth=6, random_state=42, n_jobs=-1, tree_method="hist")
xgb_model.fit(X_train_val, y_train_val)

joblib.dump(xgb_model, os.path.join(MODELS_DIR, 'stacking.joblib'))
print("Retrained stacking.joblib saved as single XGBoost.")
